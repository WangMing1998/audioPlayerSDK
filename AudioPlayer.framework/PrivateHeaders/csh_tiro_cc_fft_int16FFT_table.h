#ifndef csh_tiro_cc_fft_int16FFT_table
#define csh_tiro_cc_fft_int16FFT_table
#include "csh_tiro_cc_fft_int16FFT.h"

typedef union IBALONG { // Integer or Byte-addressable LONG
	int32_t l;				// long: Var.l
	uint16_t i[2];// u int: Var.i[0]:Var.i[1]
	uint8_t b[4]; // u char: Var.b[0]:Var.b[1]:Var.b[2]:Var.b[3]
}IBALONG;
...
#endif
//-----------------------------------------------------------------------------
// SIN表格和位翻转表格 NUM_FFT = 256
//-----------------------------------------------------------------------------
#if (NUM_FFT == 256)
const int16_t SinTable[64] =
{
...
#endif
//-----------------------------------------------------------------------------
// SIN表格和位翻转表格 NUM_FFT = 32
//-----------------------------------------------------------------------------
#if (NUM_FFT == 32)
const int16_t SinTable[8] =
{
	0x0000, 0x18F8, 0x30FB, 0x471C, 0x5A82, 0x6A6D, 0x7641, 0x7D8A
};
const uint8_t BRTable[16] =
{
	0, 8, 4, 12, 2, 10, 6, 14,
	1, 9, 5, 13, 3, 11, 7, 15
};
#endif
//-----------------------------------------------------------------------------
// SIN表格和位翻转表格 NUM_FFT = 16
//-----------------------------------------------------------------------------
#if (NUM_FFT == 16)
const int16_t SinTable[4] =
{
	0x0000, 0x30FB, 0x5A82, 0x7641
};
const uint8_t BRTable[8] =
{
	0, 4, 2, 6, 1, 5, 3, 7
};
#endif
//-----------------------------------------------------------------------------
// SIN表格和位翻转表格 NUM_FFT = 8
//-----------------------------------------------------------------------------
#if (NUM_FFT == 8)
const int16_t SinTable[2] =
{
	0x0000, 0x5A82
};
const uint8_t BRTable[4] =
{
	0, 2, 1, 3
};
#endif
//-----------------------------------------------------------------------------
// SIN表格和位翻转表格 NUM_FFT = 4
//-----------------------------------------------------------------------------
#if (NUM_FFT == 4)
const int16_t SinTable[1] =
{
	0x0000
};
const uint8_t BRTable[2] =
{
	0, 1
};
#endif
